...
## Assignment 3
All three group members run a node:

```bash
uv run assignment-3/main.py
```

Each node mines, gossips blocks to its teammates, and converges on one chain.
The process never exits; leave all three running until the server's pass email
arrives.

The `.env` file should contain:
- `KEY_FILE`: path to the private key (`.pem`) — the same key from labs 1 and 2.
- `TEAMMATES`: public keys (hex) of the other two members, comma-separated.
- `GROUP_ID`: group id from lab 2.
- `BLOCKCHAIN_COMMUNITY_ID`: 20-byte (40 hex) id of your blockchain community. All
  three members must use the same value.
- `FORCE_REGISTER` (optional): by default only the lowest-pubkey member registers
  the chain with the server. Set to `1` to force this node to register.

### Adaptive Proof of Work

Difficulty is not fixed; it retargets to hold block time steady as hashpower
joins or leaves. Every block declares its own `difficulty` (leading zero bits)
in the header, and each node recomputes the expected difficulty from the chain
and rejects any block that doesn't match.

- **Retarget.** Difficulty is adjusted from the median block timestamps over a
  sliding window, comparing the recent window against the prior one. The change
  per step is bounded so the controller tracks real shifts without overshooting.
- **Timestamp clamping.** A block's timestamp must be `>= parent` and within a
  small drift of the validator's own clock. Combined with using the *median* of
  the window, a single miner lying about timestamps cannot move the difficulty.
