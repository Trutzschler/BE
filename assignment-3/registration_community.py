...
REGISTRATION_COMMUNITY_ID: bytes = bytes.fromhex("4c616233426c6f636b636861696e323032365057")
...
class RegistrationCommunity(Community, PeerObserver):
    ...
    def send_register(self) -> None:
        ...
        self.ez_send(self.server, RegisterBlockchain(self.group_id, self.blockchain_community_id))
    ...
    def on_register_response(self, peer: Peer, response: RegisterResponse) -> None:
        if peer.public_key.key_to_bin() != REGISTRATION_SERVER_PUBLIC_KEY:
            print(f"Ignoring RegisterResponse from non-server peer {peer.public_key.key_to_bin().hex()}")
            return

        print(f"RegisterResponse: success={response.success}, message={response.message!r}")
